...
export default function PedidoDetailPage() {
  ...
  const getActiveStep = () => {
    if (order.status === 'draft' || order.status === 'pending_approval') return 0;
      ...
      <div className="w-full h-full xl:max-w-4xl mx-auto flex flex-col relative md:pt-6 md:px-8">
        ...
        <div className="bg-white rounded-[40px] p-8 shadow-xl shadow-slate-200/50 border border-slate-50 mb-6 mx-4 md:mx-0">
          ...
          <div className="relative flex justify-between">
            ...
            <div 
              className="absolute top-5 left-0 h-[2px] bg-brand-blue z-0 transition-all duration-1000"
              ...
              return (
                <div key={step.label} className="relative z-10 flex flex-col items-center gap-3">
                  <div className={`
                    ...
                    ${isActive ? 'bg-brand-blue border-brand-blue text-white shadow-lg shadow-brand-blue/30' : 'bg-white border-slate-100 text-slate-300'}
                    ${isCurrent ? 'scale-110 ring-4 ring-brand-blue/10' : ''}
                  `}>
                    <Icon size={18} />
                  </div>
                  <span className={`text-[9px] font-black uppercase tracking-tight text-center max-w-[70px] ${isActive ? 'text-slate-800' : 'text-slate-300'}`}>
                    {step.label}
                  </span>
                </div>
              );
